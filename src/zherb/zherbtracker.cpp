...
CzHERBTracker::CzHERBTracker(std::string strWalletFile)
{
...
CzHERBTracker::~CzHERBTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzHERBTracker::Init()
{
...
bool CzHERBTracker::Archive(CMintMeta& meta)
{
...
bool CzHERBTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzHERBTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzHERBTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzHERBTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzHERBTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzHERBTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzHERBTracker::GetSerialHashes()
{
...
CAmount CzHERBTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzHERBTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzHERBTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzHERBTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzHERBTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzHERBTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzHERBTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzHERBTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzHERBTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzHERBTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzHERBTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzHERBWallet* zHERBWallet)
{
    bool iszHERBWalletInitialized = (NULL != zHERBWallet);
    CMintMeta meta;
    ...
    if (! iszHERBWalletInitialized)
        zHERBWallet = new CzHERBWallet(strWalletFile);
    meta.isSeedCorrect = zHERBWallet->CheckSeed(dMint);
    if (! iszHERBWalletInitialized)
        delete zHERBWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzHERBTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzHERBTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzHERBTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzHERBTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzHERBTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzHERBTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzHERBWallet* zHERBWallet = new CzHERBWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zHERBWallet);
        }
        delete zHERBWallet;
        LogPrint("zero", "%s: added %d dzherb from DB\n", __func__, listDeterministicDB.size());
    }
