...
#include "ui_zherbcontroldialog.h"
...
void ZPivControlDialog::updateList()
{
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        {
            LOCK(pwalletMain->zherbTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zherbTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zHERB.";
            else if (!mint.isSeedCorrect)
                strReason = "The zHERB seed used to mint this zHERB is not the same as currently hold in the wallet";
            else
