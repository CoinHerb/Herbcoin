...
public:
    ...
    bool WriteZHERBSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZHERBSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZHERBSeed_deprecated(uint256& seed);
    bool EraseZHERBSeed();
    bool EraseZHERBSeed_deprecated();

    bool WriteZHERBCount(const uint32_t& nCount);
    bool ReadZHERBCount(uint32_t& nCount);
