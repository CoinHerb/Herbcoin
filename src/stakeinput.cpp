...
// The zHERB block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zHERB is a hash of the serial
...
{
    //Create an output returning the zHERB that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZHERBOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zHERB output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zHERB", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZHERBOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zHERB output", __func__);
...
{
    CzHERBTracker* zherbTracker = pwallet->zherbTracker.get();
    CMintMeta meta;
    if (!zherbTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zherbTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a HERB stake is the outpoint
