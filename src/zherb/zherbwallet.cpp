...
CzHERBWallet::CzHERBWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zherb seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZHERBSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZHERBSeed_deprecated()) {
                    LogPrintf("%s: Updated zHERB seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zherb seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zherb wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzHERBWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZHERBCount(nCountLastUsed);
    else if (!walletdb.ReadZHERBCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzHERBWallet::Lock()
{
    seedMaster = 0;
}

void CzHERBWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzHERBWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZHERB(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzHERBWallet::LoadMintPoolFromDB()
{
...
void CzHERBWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzHERBWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzHERBWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zherbTracker->HasPubcoinHash(pMint.first)) {
...
bool CzHERBWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZHERB(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zherbTracker which also adds to database
    pwalletMain->zherbTracker->Add(dMint, true);
...
void CzHERBWallet::SeedToZHERB(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzHERBWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzHERBWallet::GenerateDeterministicZHERB(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzHERBWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzHERBWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzHERBWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
