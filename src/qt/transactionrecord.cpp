...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.HasZerocoinSpendInputs()) {
        ...
        if (wtx.HasZerocoinPublicSpendInputs()) {
            ...
            if (!ZHERBModule::ParseZerocoinPublicSpend(wtx.vin[0], wtx, state, publicSpend)){
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.HasZerocoinSpendInputs() && (fZSpendFromMe || wallet->zherbTracker->HasMintTx(hash))) {
            //zHERB stake reward
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::StakeZHERB;
...
bool IsZHERBType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::StakeZHERB:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || type == TransactionRecord::StakeZHERB || type == TransactionRecord::MNReward) {
